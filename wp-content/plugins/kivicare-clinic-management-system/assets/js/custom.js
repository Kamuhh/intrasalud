...
function displayMessage(message, duration = 10000) {
  window.Snackbar.show({
    ...
    pos: 'top-right',
    duration: duration,
    actionText: window.__kivicarelang.common.dismiss,
  });
}
...
function kivicare_generate_microsoft(e) {
  try {
    // host según el proveedor
    var host = e.provider === 'outlook' ? 'outlook.live.com' : 'outlook.office.com';
    var url =
      'https://' +
      host +
      '/calendar/0/deeplink/compose?path=%2Fcalendar%2Faction%2Fcompose&rru=addevent';

    // Rango de tiempo (NO reutilizar variable t)
    var timeRange = kivicare_generate_time(e, 'delimiters', 'microsoft');
    if (!timeRange) {
      throw new Error('Failed to generate time for Microsoft Outlook calendar.');
    }
    url += '&startdt=' + timeRange.start + '&enddt=' + timeRange.end;
    if (timeRange.allday) {
      url += '&allday=true';
    }
    ...
    // Metadatos
    if (e.name && e.name !== '') {
      url += '&subject=' + encodeURIComponent(e.name);
    }
    if (e.location && e.location !== '') {
      url += '&location=' + encodeURIComponent(e.location);
    }
    if (e.description && e.description !== '') {
      url += '&body=' + encodeURIComponent(e.description.replace(/\n/g, '<br>'));
    }
...
function kivicare_generate_teams(e) {
  ...
  null != e.name && '' != e.name && (t += '&subject=' + encodeURIComponent(e.name)),
    null != e.location &&
      '' != e.location &&
      ((a = encodeURIComponent(e.location)), (t += '&location=' + a), (a += ' // ')),
    null != e.description &&
      '' != e.description &&
...
function kivicare_generate_ical(t) {
  let e = new Date();
  e = e.toISOString().replace(/\..../g, '').replace(/[^a-z0-9]/gi, '');
...
function kivicare_add_to_calendar_url(config, type) {
  switch (type) {
    ...
    case 'microSoftOutlookLive':
      // Nota: ahora kivicare_generate_microsoft ya no recibe segundo parámetro
      config.provider = 'outlook';
      kivicare_generate_microsoft(config);
      break;
    case 'microSoftOutlookoffice':
      config.provider = 'office';
      kivicare_generate_microsoft(config);
      break;
    case 'microSoftTeam':
...
function hideLeftNavItems() {
  ...
  const root =
    document.querySelector('[class*="sidebar"], nav, .kc-sidebar, .kc_left_sidebar') || document;
...
// Helper once (usado arriba y abajo)
function once(fn) {
  let done = false;
  return function () {
    if (!done) {
...
(function () {
  ...
  function openSummary(encounterId) {
    const url =
      ajaxData.ajaxurl +
      '?action=kivi_route&route=patient_encounter_summary&id=' +
    ...
    document.querySelectorAll('[data-encounter-id]').forEach(function (row) {
      ...
      btn.textContent =
        (window.__kivicarelang &&
          __kivicarelang.common &&
          __kivicarelang.common.summary) ||
        'Resumen';
  ...
  const initSummary = onceLocal(function () {
    injectButtons();
